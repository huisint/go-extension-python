...
from pip._vendor.packaging import utils as pkgutils
...
class TestGopyCompiler___init__(TestCase):
    ...
    def test_go_command(self) -> None:
        assert _compiler.GoPyCompiler.go_command == "go"
        compiler = _compiler.GoPyCompiler(go_command="gogo")
        self.assertEqual(compiler.go_command, "gogo")

    def test_verbose(self) -> None:
        assert _compiler.GoPyCompiler.verbose is False
        compiler = _compiler.GoPyCompiler(verbose=True)
        self.assertTrue(compiler.verbose)

    def test_dry_run(self) -> None:
        assert _compiler.GoPyCompiler.dry_run is False
        compiler = _compiler.GoPyCompiler(dry_run=True)
        self.assertTrue(compiler.dry_run)

    def test_force(self) -> None:
        assert _compiler.GoPyCompiler.force is False
        compiler = _compiler.GoPyCompiler(force=True)
        self.assertTrue(compiler.force)

    def test_inplace(self) -> None:
        assert _compiler.GoPyCompiler.force is False
        compiler = _compiler.GoPyCompiler(inplace=True)
        self.assertTrue(compiler.inplace)

    def test_env(self) -> None:
        assert _compiler.GoPyCompiler.env.get("LD_LIBRATY_PATH", None) is None
        compiler = _compiler.GoPyCompiler()
        self.assertTrue(hasattr(compiler, "env"))
        self.assertIsNotNone(compiler.env.get("LD_LIBRATY_PATH", None))


@mock.patch("go_extension.compiler._pybindgen_is_installed")
...
class TestGoPyCompiler_install_build_tools(TestCase):
    ...
    def test_all_installed(
        ...
        pybindgen_is_installed_mock: mock.Mock,
    ) -> None:
        gopy_is_installed_mock.return_value = True
        goimports_is_installed_mock.return_value = True
        pybindgen_is_installed_mock.return_value = True
        compiler = _compiler.GoPyCompiler()
        compiler.install_build_tools()
        self.assertIsNotNone(shutil.which(compiler.go_command))
        try:
            gopy_is_installed_mock.assert_called_once()
            goimports_is_installed_mock.assert_called_once()
            pybindgen_is_installed_mock.assert_called_once()
    ...
    def test_go_command_not_exist(
        ...
        unexist_go_command = "gogo"
        assert shutil.which(unexist_go_command) is None
        compiler = _compiler.GoPyCompiler(go_command=unexist_go_command)
    ...
    def test_install_pybindgen(
        self,
        gopy_is_installed_mock: mock.Mock,
        goimports_is_installed_mock: mock.Mock,
        pybindgen_is_installed_mock: mock.Mock
    ) -> None:
        pybindgen_is_installed_mock.return_value = False
        compiler = _compiler.GoPyCompiler()
        compiler.install_build_tools()
        env = metadata.get_default_environment()
        pip_installed = {dist.canonical_name: dist for dist in env._iter_distributions()}
        self.assertTrue(pkgutils.canonicalize_name("pybindgen") in pip_installed)
        pybindgen_is_installed_mock.assert_called_once()
...
class TestGoPyCompiler_generate(TestCase):
    ...
    def setUp(self) -> None:
        self.compiler = _compiler.GoPyCompiler()
